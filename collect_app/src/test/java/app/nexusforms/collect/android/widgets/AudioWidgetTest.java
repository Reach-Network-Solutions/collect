...
import app.nexusforms.android.widgets.AudioWidget;
...
import app.nexusforms.collect.android.widgets.support.FakeQuestionMediaManager;
...
import static app.nexusforms.collect.android.support.RobolectricHelpers.setupMediaPlayerDataSource;
...
public class AudioWidgetTest {
    ...
    public void whenPromptDoesNotHaveAnswer_showsButtons() {
        AudioWidget widget = createWidget(QuestionWidgetHelpers.promptWithAnswer(null));
    ...
    public void whenPromptHasAnswer_showsAudioController() throws Exception {
        ...
        AudioWidget widget = createWidget(QuestionWidgetHelpers.promptWithAnswer(new StringData(answerFile.getName())));
    ...
    public void usingReadOnlyOption_doesNotShowCaptureAndChooseButtons() {
        AudioWidget widget = createWidget(QuestionWidgetHelpers.promptWithReadOnly());
    ...
    public void whenWidgetIsNew_chooseSoundButtonIsNotShown() {
        FormEntryPrompt prompt = QuestionWidgetHelpers.promptWithReadOnly();
    ...
    public void deleteFile_setsFileAsideForDeleting() throws Exception {
        File answerFile = questionMediaManager.addAnswerFile(File.createTempFile("blah", ".mp3"));
        FormEntryPrompt prompt = QuestionWidgetHelpers.promptWithAnswer(new StringData(answerFile.getName()));
    ...
    public void setData_whenPromptHasDifferentAudioFile_deletesOriginalAnswer() throws Exception {
        ...
        FormEntryPrompt prompt = QuestionWidgetHelpers.promptWithAnswer(new StringData(originalFile.getName()));
        when(prompt.getIndex()).thenReturn(formIndex);
    ...
    public void usingReadOnlyOptionShouldMakeAllClickableElementsDisabled() throws Exception {
        File answerFile = questionMediaManager.addAnswerFile(File.createTempFile("blah", ".mp3"));
        AudioWidget widget = createWidget(QuestionWidgetHelpers.promptWithReadOnlyAndAnswer(new StringData(answerFile.getName())));
        widget.binding.audioPlayer.audioController.binding.remove.performClick();
    ...
    public void whenReadOnlyOverrideOptionIsUsed_shouldAllClickableElementsBeDisabled() throws Exception {
        File answerFile = questionMediaManager.addAnswerFile(File.createTempFile("blah", ".mp3"));
        AudioWidget widget = createWidget(QuestionWidgetHelpers.promptWithAnswer(new StringData(answerFile.getName())), true);
        widget.binding.audioPlayer.audioController.binding.remove.performClick();
