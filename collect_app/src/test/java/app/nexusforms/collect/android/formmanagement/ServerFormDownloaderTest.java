...
import static app.nexusforms.collect.android.support.FormUtils.buildForm;
import static app.nexusforms.collect.android.support.FormUtils.createXFormBody;
...
public class ServerFormDownloaderTest {
    ...
    public void downloadsAndSavesForm() throws Exception {
        String xform = FormUtils.createXFormBody("id", "version");
        ...
        File formFile = new File(StoragePathProvider.getAbsoluteFilePath(formsDir.getAbsolutePath(), form.getFormFilePath()));
        assertThat(formFile.exists(), is(true));
        assertThat(new String(FileUtils.read(formFile)), is(xform));
    }
    ...
    public void whenFormToDownloadIsUpdate_savesNewVersionAlongsideOldVersion() throws Exception {
        ...
        String xformUpdate = FormUtils.createXFormBody("id", "updated");
        ...
        allForms.forEach(f -> {
            File formFile = new File(StoragePathProvider.getAbsoluteFilePath(formsDir.getAbsolutePath(), f.getFormFilePath()));
            assertThat(formFile.exists(), is(true));
        });
    }
    ...
    public void whenFormHasMediaFiles_downloadsAndSavesFormAndMediaFiles() throws Exception {
        ...
        assertThat(new String(FileUtils.read(mediaFile1)), is("contents1"));

        File mediaFile2 = new File(form.getFormMediaPath() + "/file2");
        assertThat(mediaFile2.exists(), is(true));
        assertThat(new String(FileUtils.read(mediaFile2)), is("contents2"));
    }
    ...
    public void whenFormIsSoftDeleted_unDeletesForm() throws Exception {
        String xform = FormUtils.createXFormBody("deleted-form", "version");
        Form form = FormUtils.buildForm("deleted-form", "version", getFormFilesPath(), xform)
                .deleted(true)
                .build();
    ...
    public void whenMultipleFormsWithSameFormIdVersionDeleted_reDownloadUnDeletesFormWithSameHash() throws Exception {
        ...
        Form form2 = FormUtils.buildForm("deleted-form", "version", getFormFilesPath(), xform2)
                .deleted(true)
                .build();
    ...
    public void whenDownloadingFormWithVersion_andId_butNotHashOnDevice_logsAnalytics() throws Exception {
        ...
        Form form = FormUtils.buildForm("form", "version", getFormFilesPath(), xform).build();
        formsRepository.save(form);

        String xform2 = FormUtils.createXFormBody("form2", "version", "A different title");
        Form form2 = FormUtils.buildForm("form", "version", getFormFilesPath(), xform2).build();
        ...
        verify(mockAnalytics).logFormEvent(AnalyticsEvents.DOWNLOAD_SAME_FORMID_VERSION_DIFFERENT_HASH, formIdHash);
    }
