...
public class IntentGroupTest {
    ...
    public void externalApp_ShouldPopulateFields() throws IOException {
        ...
        onView(withText("This is buttonText")).perform(NestedScrollToAction.nestedScrollTo(), click());
    ...
    public void externalApp_ShouldNotPopulateFieldsIfAnswersAreNull() {
        ...
        onView(CustomMatchers.withIndex(withClassName(endsWith("EditText")), 1)).check(matches(withText("")));
    ...
    private void assertVideoWidgetWithoutAnswer() {
        onView(withText(is("Video external"))).perform(NestedScrollToAction.nestedScrollTo()).check(matches(isDisplayed()));
    ...
    private void assertImageWidgetWithAnswer() {
        onView(withTagValue(is("ImageView"))).perform(NestedScrollToAction.nestedScrollTo()).check(matches(isDisplayed()));
        onView(withId(R.id.capture_image)).check(doesNotExist());
        onView(withId(R.id.choose_image)).check(doesNotExist());
    }

    private void assertAudioWidgetWithAnswer() {
        onView(withId(R.id.audio_controller)).perform(NestedScrollToAction.nestedScrollTo()).check(matches(isDisplayed()));
    }

    private void assertVideoWidgetWithAnswer() {
        onView(withId(R.id.play_video)).perform(NestedScrollToAction.nestedScrollTo()).check(matches(isDisplayed()));
        onView(withId(R.id.play_video)).check(matches(isEnabled()));
    }

    private void assertFileWidgetWithAnswer() {
        onView(withTagValue(is("ArbitraryFileWidgetAnswer"))).perform(NestedScrollToAction.nestedScrollTo()).check(matches(isDisplayed()));
    }
    ...
    private Uri createTempFile(String name, String extension) throws IOException {
        ...
        FileUtils.copyFileFromAssets("media" + File.separator + name + "." + extension, file.getPath());
