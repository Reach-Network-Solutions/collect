...
import app.nexusforms.android.forms.FormUtils;
...
public class FormLoadingUtils {
    ...
    public static void copyFormToStorage(String formFilename, List<String> mediaFilePaths, boolean copyToDatabase, String copyTo) throws IOException {
        ...
        if (copyToDatabase) {
            FormUtils.setupReferenceManagerForForm(ReferenceManager.instance(), FileUtils.getFormMediaDir(new File(pathname)));
    ...
    private static void saveFormToDatabase(File outFile) {
        ...
        v.put(FormsProviderAPI.FormsColumns.FORM_FILE_PATH, new StoragePathProvider().getRelativeFormPath(outFile.getAbsolutePath()));
        v.put(FormsProviderAPI.FormsColumns.FORM_MEDIA_PATH, new StoragePathProvider().getRelativeFormPath(FileUtils.constructMediaPath(outFile.getAbsolutePath())));
        ...
        Collect.getInstance().getContentResolver().insert(FormsProviderAPI.FormsColumns.CONTENT_URI, v);
    }
