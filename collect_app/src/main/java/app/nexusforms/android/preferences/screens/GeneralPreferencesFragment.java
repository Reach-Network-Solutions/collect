...
import static app.nexusforms.android.preferences.keys.AdminKeys.KEY_MAPS;
...
public class GeneralPreferencesFragment extends BaseGeneralPreferencesFragment implements Preference.OnPreferenceClickListener {
    ...
    private void setPreferencesVisibility() {
        ...
        boolean mapsScreenEnabled = settingsProvider.getAdminSettings().getBoolean(KEY_MAPS);
    ...
    private boolean hasAtleastOneSettingEnabled(Collection<String> keys) {
        for (String key : keys) {
            boolean value = settingsProvider.getAdminSettings().getBoolean(key);
            if (value) {
                return true;
            }
        }
