...
public class VolumeBar extends LinearLayout {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed && getHeight() > 0) {
            ...
            int marginSize = ViewUtils.pxFromDp(getContext(), 4);
    ...
    private int getBestPipSize(int availableWidth) {
        if (ViewUtils.dpFromPx(getContext(), availableWidth) >= 164) {
            return ViewUtils.pxFromDp(getContext(), 24);
        } else {
            return ViewUtils.pxFromDp(getContext(), 20);
        }
    }
