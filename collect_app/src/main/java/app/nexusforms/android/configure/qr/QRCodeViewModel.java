...
class QRCodeViewModel extends ViewModel {
    ...
    private Collection<String> includedKeys = asList(AdminKeys.KEY_ADMIN_PW, GeneralKeys.KEY_PASSWORD);
    ...
    private void generateQRCode() {
        scheduler.immediate(
                ...
                qrCode -> {
                    ...
                    boolean serverPasswordSet = !generalSettings.getString(GeneralKeys.KEY_PASSWORD).isEmpty();
                    boolean adminPasswordSet = !adminSettings.getString(AdminKeys.KEY_ADMIN_PW).isEmpty();

                    if (serverPasswordSet || adminPasswordSet) {
                        if (serverPasswordSet && includedKeys.contains(GeneralKeys.KEY_PASSWORD) && adminPasswordSet && includedKeys.contains(AdminKeys.KEY_ADMIN_PW)) {
                            warning.setValue(R.string.qrcode_with_both_passwords);
                        } else if (serverPasswordSet && includedKeys.contains(GeneralKeys.KEY_PASSWORD)) {
                            warning.setValue(R.string.qrcode_with_server_password);
                        } else if (adminPasswordSet && includedKeys.contains(AdminKeys.KEY_ADMIN_PW)) {
