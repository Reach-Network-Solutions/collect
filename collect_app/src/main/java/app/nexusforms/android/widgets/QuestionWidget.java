...
import app.nexusforms.android.formentry.questions.QuestionTextSizeHelper;
...
import app.nexusforms.android.widgets.interfaces.Widget;
import app.nexusforms.android.widgets.items.SelectImageMapWidget;
...
public abstract class QuestionWidget extends FrameLayout implements Widget {
    ...
    private void setupQuestionLabel(AudioVideoImageTextLabel label, FormEntryPrompt prompt) {
        label.setTag(FormMediaUtils.getClipID(prompt));
        ...
        String playableAudioURI = FormMediaUtils.getPlayableAudioURI(prompt, referenceManager);
        try {
            if (imageURI != null) {
            ...
            if (playableAudioURI != null) {
                ...
                analytics.logEvent(AnalyticsEvents.PROMPT, "AudioLabel", questionDetails.getFormAnalyticsID());
            }
        } catch (InvalidReferenceException e) {
            Timber.d(e, "Invalid media reference due to %s ", e.getMessage());
        }

        label.setPlayTextColor(FormMediaUtils.getPlayColor(formEntryPrompt, themeUtils));
    }
     ...
     * {@link QuestionMediaManager} to access media attached
     * to the instance.
     */
    @Nullable
    @Deprecated
